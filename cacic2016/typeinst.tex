...
Posteriormente, la aplicacion utiliza el algoritmo line segment detector (LSD) [REFERENCIA] para obtener un conjunto de segmentos de linea que representan a la imagen original. Entonces, para la medición de diámetros de fibras, se utiliza esta representación como entrada, y su descripción es como sigue:
...
\begin{enumerate}
\item Por cada segmento de linea en el conjunto se calcula la ubicación de 3 puntos (x,y):
...
\item Si no se descarta entonces el segmento de linea paralelo encontrado es opuesto al original en la fibra a la cual pertenecen, y se procede a realizar el calculo de distancia entre el punto original y la intersección. Esta medida es catalogada como diámetro tentativo.
