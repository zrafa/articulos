...
\author{
\IEEEauthorblockN{Rafael Zurita, Rodolfo del Castillo, Miriam Lechner, Eduardo Grosclaude \\ \\
...
Then, we modified an example program from Cirrus, in ARM assembler language,
which is not greater than 2048 bytes once built. The modifications
lets discover the missing information about the architecture, and
it fits the size restrictions needed by the the serial downloader feature
...
In particular, our program looks for the phisical addresses of the internal
FLASH memory, because we are interested on the proper addresses where
the software components are installed
(bootloader, Linux kernel and the root filesystem). 
In order to achieve that goal, the program looks for the ``CRUS'' word, reading consecutive bytes
starting from the 0x60000000 physical address.
We choosed the 0x600000000 address using possible values from the SOC documentation.
The ``CRUS'' word is also defined by the documentation, as the key word
for the CPU to identify the start boot address, when the CPU tries different addresses for booting code.
...
We sent the little program using the serial downloader feature of the Boot ROM process,
and it found the ``CRUS'' word on 
FLASH memory at the 0x60001000 address. This address ensures us that the CPU
will run any code we write after that, in the booting proccess.
For example, that is a proper place to write the bootloader component software.
...
We then modified the program with a similar behaviour, but the goal
 this time was to find the ending unkown RAM physical address.
After few trial an error tests, the program found that limit, and we used
it for doing proper modifications to the U-boot bootloader RAM settings.
...
Once we knew how to execute small programs after a
hadrware reset event, and having at hand the RAM and FLASH
addresses, we were ready to deploy an alternative Linux
embedded system on the device. Our first step to accomplish
this was to prepare a small bootloader, sent over the serial line
to RAM. In particular, we choosed u-boot configured for this
particular device.
