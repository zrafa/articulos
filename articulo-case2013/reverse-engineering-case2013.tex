...
When reusing a closed-box hardware and software system, 
it is needed either a written complete documentation of device specifications, or to look at how it works using
different experiments and tools.
This is called reverse enginerring, and that is the method used.
...
Because this method is illegal in some countries, it is needed
verify if it is allowed by the license restrictions of the propietary product and by the local law. In this case, reverse engineering was used for reusing the hardware for arbitrary purposes, which matches the terms of ``Ley Argentina N° 24.240 de Defensa del Consumidor'' law, provided the product has been purchased legally.
...
\subsection*{Comunicación con la CPU}

El SOC Cirrus EP9307 tiene una ROM de arranque\footnote{boot ROM}.
En un reinicio (en inglés reset) del SOC, la CPU (ARM920t) comienza a ejecutar código en la dirección cero.

Por otro lado, el manual del SOC indica que éste utiliza los controles de la ``configuración del hardware'' para seleccionar
...
Afortunadamente, en un reinicio, el Cirrus EP9307, en conjunción
...
Se utilizó ingeniería inversa para entender la arquitectura, funcionamiento
